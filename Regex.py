...
postcode = 'DN312QP'
first_line_address = '79 Elsenham Road'
second_line_address = 'Grimsby'
...
for address in addresses:
    addresslist.append((address.text).strip().replace(",", ""))

    # print(addresslist)
    # print(first_line_address.strip().replace(",", ""))

    occurences = sum((first_line_address.strip().replace(",", "")) in s for s in addresslist)
...
if occurences == 1:
    ...
    for address in addresses:
        if firstandsecondline in (address.text).strip().replace(",", ""):
            propertyURL = ('https://find-energy-certificate.service.gov.uk/'+address['href'])
